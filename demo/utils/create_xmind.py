...
def gen_my_xmind_file(name, post_form, form_items , file_dic):
    ...
    design_sheet2(sheet1, name, post_form, form_items,file_dic)
...
def design_sheet2(sheet1, name, post_form, form_items=None , file_list=None):
    # ***** first sheet *****
    sheet1.setTitle(name)  # set its title
    ...
    for item in form_items:
        if(item['type'] == "text" and post_form.get(item["id"])): #只有text才直接写入
            #print(item)
            sub_topic1 = root_topic1.addSubTopic()
